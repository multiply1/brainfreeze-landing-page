...
import {
  ...
  Menu,
  Twitter,
  Snowflake, // Käytetään placeholderina logolle
  ChevronDown,
} from "lucide-react";

// --- MOCK COMPONENTS FOR PREVIEW (Korvaa nämä omilla importeillasi) ---
const useAuth = () => ({
  user: null, // Vaihda { name: "User" } testataksesi kirjautunutta tilaa
  signOut: () => console.log("Sign out clicked"),
});

// Yksinkertaistettu Button
const Button = ({ className, variant, size, onClick, children }) => {
  // "Jäinen" perustyyli napille
  const iceStyle =
...
// Yksinkertaistetut UI komponentit
...
const Header = () => {
  ...
  const [isCommunityOpen, setIsCommunityOpen] = useState(false); // Mobiili state
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false); // Sheet state

  const isHomePage = location.pathname === "/";
  const isAuthPage = location.pathname === "/auth";

  return (
    // Outer container: Fixed position, full transparency, centered content
    <motion.header
    ...
    >
      {/* THE CAPSULE: Tämä on Koodi 1:n tyylinen "BrainFreeze" container.
        Se yhdistää Koodi 2:n logiikan Koodi 1:n ulkoasuun.
        pointer-events-auto palauttaa klikattavuuden tähän elementtiin.
      */}
      <div
        className="
        pointer-events-auto
        ...
        bg-[#050b14]/60 backdrop-blur-xl
        border border-white/10
        shadow-[0_0_30px_rgba(8,145,178,0.3)]
      "
      >
        {/* ================= 1. VASEN: LOGO (BrainFreeze Style) ================= */}
        <div className="flex items-center gap-4 shrink-0">
          <Link to="/" className="flex items-center gap-3 group">
            {/* Placeholder logo - korvaa tämä: <img src={logo} ... /> */}
            ...
            <div className="flex flex-col md:block leading-tight">
              ...
              {/* Beta tagi koodista 2, tyyliteltynä */}
        ...
        {/* ================= 2. KESKI: NAVIGAATIO (Code 2 Links, Code 1 Style) ================= */}
        {/* Käytämme 'absolute' positiota keskittämiseen kuten koodissa 1, 
           jotta se pysyy keskellä riippumatta vasemman/oikean puolen leveydestä.
           Piilotetaan mobiilissa.
        */}
        {!isAuthPage && (
          <div className="hidden md:flex items-center gap-8 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            ...
            {/* Community Dropdown (Code 2 Logic) */}
            <DropdownMenu>
              <DropdownMenuTrigger>
        ...
        {/* ================= 3. OIKEA: ACTION NAPPI (Code 2 Logic, Code 1 Visuals) ================= */}
        <div className="flex items-center gap-4 shrink-0">
          {/* Desktop Actions */}
          {!isAuthPage && (
            <div className="hidden md:block">
              {user ? (
                ...
                <Link to="/auth">
                  {/* Tämä nappi on tyylitelty näyttämään Koodi 1:n "Ice Buttonilta" */}
                  <Button className="shadow-[0_0_20px_rgba(8,145,178,0.5)] border-cyan-200/30">Start Creating</Button>
                </Link>
              )}
            </div>
          )}

          {/* Mobiili Menu Trigger */}
          <Sheet>
